...
import { DraftReviewQueuePage } from './DraftReviewQueuePage';
import { PublishedHistoryPage } from './PublishedHistoryPage';

type Props = { session: AuthSession };

export function AdminDashboardPage({ session }: Props) {
  if (session.role !== 'moderator' && session.role !== 'admin') return <p>Admin dashboard requires moderator/admin role.</p>;
  ...
  return (
    <section>
      ...
      <div className="panel" style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap' }}>
        ...
        <NavLink to="drafts">Draft Review</NavLink>
        <NavLink to="published">Published History</NavLink>
      ...
      <Routes>
        ...
        <Route path="drafts" element={<DraftReviewQueuePage session={session} />} />
        <Route path="published" element={<PublishedHistoryPage session={session} />} />
