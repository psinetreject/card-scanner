...
import type { Alias, AuthSession, Card, OutboxProposal, Print, SyncState, UserCollectionEntry, UserScan } from '../core/types';
...
export class IndexedDbStorageService implements IStorageService {
  private dbPromise = openDB<ScannerDb>('ygo-scanner', 2, {
  ...
  async importSnapshot(snapshot: LocalBundle, mode: 'replace' | 'merge'): Promise<void> {
    const db = await this.dbPromise;
    const stores: (keyof ScannerDb)[] = ['cache_cards', 'cache_prints', 'cache_aliases', 'user_collection', 'user_scans', 'outbox_proposals'];
